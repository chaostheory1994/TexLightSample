...
void setup_flashlight() {
    const GLfloat cutoff = 50.0f;
    /* FlashLight Properties */
    GLfloat flash_amb [] = {0.2, 0.2, 0.2, 1.0};
    GLfloat flash_diffuse [] = {0.8, 0.8, 0.8, 1.0};
...
void my_display(void) {
    ...
    //GLfloat center_pos[] = {0, 0, 0, 0};
    GLfloat center_pos[] = {fl.pos[0], fl.pos[1], fl.pos[2], 1};
    GLfloat lookat_pos [] = {fl.pos[0] + cos((fl.angle * M_PI) / 180), fl.pos[1], fl.pos[2] + sin((fl.angle * M_PI) / 180)};
    //GLfloat lookat_pos[] = {1, 0, 0};
    ...
    if (isFirstPerson) {
        gluLookAt(fl.pos[0], fl.pos[1], fl.pos[2],
                ...
                0, 1, 0);
    } else gluLookAt(20.0, 20.0, 100.0, // x,y,z coord of t1he camera 
    ...
    glTranslatef(lookat_pos[0], lookat_pos[1], lookat_pos[2]);
    glutSolidCube(0.1);
    glPopMatrix();
    ...
    while (curr != NULL){
    	glPushMatrix();
        center_pos[0] = curr->pos[0];
        center_pos[1] = curr->pos[1];
        center_pos[2] = curr->pos[2];
    	glRotatef(curr->angle, 0, 1, 0);
    	glLightfv(GL_LIGHT0 + i, GL_POSITION, center_pos);
